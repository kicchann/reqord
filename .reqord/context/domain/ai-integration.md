...
1. **context.json** - プロジェクトメタデータ（必須）
2. **product.json** - プロダクトビジョン・スコープ（必須）
3. **technical.json** - 技術スタック・設計原則（必須）
4. **structure.json** - 命名規則・アーキテクチャルール（タスクに応じて）
