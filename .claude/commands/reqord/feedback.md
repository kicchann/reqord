---
description: Feedback運用コマンド（同期・分類・リンク・クローズ）
...
flag のライフサイクル:
  ...
  解消 → req/spec の修正・再承認後に手動削除（※CLIコマンド未実装）
```
...
```
⚠ 以下の req/spec に feedback-review flag が残っています（別途修正・解消が必要）:
...
| # | 改善点 | 影響度 | 備考 |
|---|--------|--------|------|
| 1 | `feedback resolve` コマンドがない | high | flag 解消時に直接編集が必要。リンク先 req/spec から feedback-review flag を除去し、feedback の linkedTo.resolved に記録する機能 |
| 2 | `feedback unlink` コマンドがない | medium | 誤リンク時に直接編集が必要 |
| 3 | `feedback close` 時に残存 flag の警告表示がない | medium | flag 放置しがち |
